...
# Clean, fixed LSTM Seq2Seq with proper Train / Val / Test (+ optional TDA)
...
def train_lstm(cfg, train_loader, val_loader, test_loader, src_vocab, tgt_vocab, device):
    ...
    patience, counter, best_bleu = 3, 0, None
    history = []
    ...
    for epoch in range(cfg.NUM_EPOCHS):
        ...
        for src, lens, tgt, tda in tqdm(train_loader, desc=f"Train Epoch {epoch+1}"):
        ...
        with torch.no_grad():
            for src, lens, tgt, tda in train_loader:
                ...
                for t in range(1, tgt.size(1)):
                    out, h = decoder(inp, h)
                    inp = out.argmax(1)
                    sent.append(str(inp[0].item()))

                train_preds.append(" ".join(sent))
                train_refs.append(" ".join(map(str, tgt[0].cpu().numpy())))

        train_bleu = sacrebleu.corpus_bleu(train_preds, [[r] for r in train_refs]).score / 100

        # ------------------- VALIDATION -------------------
        preds, refs, val_loss_total = [], [], 0
        with torch.no_grad():
            for src, lens, tgt, tda in val_loader:
                src, tgt, lens = src.to(device), tgt.to(device), lens.to(device)
                tda = tda.to(device) if tda is not None else None
                ...
                for t in range(1, tgt.size(1)):
                    out, h = decoder(inp, h)
                    val_loss_total += loss_fn(out, tgt[:, t]).item()
                    inp = out.argmax(1)
                    sent.append(str(inp[0].item()))

                preds.append(" ".join(sent))
                refs.append(" ".join(map(str, tgt[0].cpu().numpy())))

        val_loss = val_loss_total / len(val_loader)
        val_bleu = sacrebleu.corpus_bleu(preds, [[r] for r in refs]).score / 100
    ...
    test_loss_total, preds, refs = 0, [], []
    with torch.no_grad():
        for src, lens, tgt, tda in test_loader:
            src, tgt, lens = src.to(device), tgt.to(device), lens.to(device)
            tda = tda.to(device) if tda is not None else None
            ...
            for t in range(1, tgt.size(1)):
                out, h = decoder(inp, h)
                test_loss_total += loss_fn(out, tgt[:, t]).item()
                inp = out.argmax(1)
                sent.append(str(inp[0].item()))
